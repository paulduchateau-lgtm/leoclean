...
describe("dansLaFenetre", () => {
  ...
  it("ouvre vingt-quatre heures avant", () => {
    expect(
      dansLaFenetre(mission, new Date("2026-09-09T08:00:00Z")),
    ).toBe(true);
  });

  it("reste fermée une minute plus tôt", () => {
    expect(
      dansLaFenetre(mission, new Date("2026-09-09T07:59:00Z")),
    ).toBe(false);
  });

  it("est ouverte pendant la mission", () => {
    expect(
      dansLaFenetre(mission, new Date("2026-09-10T09:30:00Z")),
    ).toBe(true);
  });

  it("reste ouverte deux heures après la fin", () => {
    expect(
      dansLaFenetre(mission, new Date("2026-09-10T13:00:00Z")),
    ).toBe(true);
  });

  it("se ferme au-delà", () => {
    expect(
      dansLaFenetre(mission, new Date("2026-09-10T13:01:00Z")),
    ).toBe(false);
  });
  ...
  it("reste fermée des semaines avant et après", () => {
    expect(dansLaFenetre(mission, new Date("2026-08-01T08:00:00Z"))).toBe(false);
    expect(dansLaFenetre(mission, new Date("2026-10-01T08:00:00Z"))).toBe(false);
  });
